...
# function for pre-processing:  This is adopted from Framelet code, i.e., Xuebin Zheng's version.
# Here is the corrected version.  Now instead of L, we use A^TA
def get_operator2(AtA, DFilters, n, s, J, Lev):
    r = len(DFilters)
    c = [None] * r
    for j in range(r):
    ...
    # Fast Tight Frame Decomposition (FTFD)
    FD1 = sparse.identity(AtA.shape[0])
    dd = dict()
    for l in range(Lev):
        for j in range(r):
            T0F = FD1
            T1F = ((s ** (-J - l) / a) * AtA) @ T0F - T0F
            dd[j, l] = (1 / 2) * c[j][0] * T0F + c[j][1] * T1F
            for k in range(2, n):
                TkF = ((2 / a * s ** (-J - l)) * AtA) @ T1F - 2 * T1F - T0F
                T0F = T1F
                T1F = TkF
                dd[j, l] += c[j][k] * TkF
        FD1 = dd[0, l]
...
class SVDUFGConv(nn.Module):
    def __init__(self, in_features, out_features, r, Lev, num_nodes, shrinkage, sigma, activation = None, bias=True):
    ...
    def forward(self, x, d_list, A):
        ...
        for block_idx in range(self.Lev - 1, self.r * self.Lev):
            ...
            if block_idx == self.Lev - 1:  # low frequency block
                ...
                x_shrink = torch.cat((x_shrink,
                                      waveletShrinkage(
                                          x[block_idx * self.num_nodes:(block_idx + 1) * self.num_nodes, :],
                                          simpleLambda(
                                              x[block_idx * self.num_nodes:(block_idx + 1) * self.num_nodes, :],
        ...
        if d_list[0].is_sparse:
             x_shrink = torch.sparse.mm(A, torch.sparse.mm(torch.cat(d_list[self.Lev - 1:], dim=1), x_shrink))
        else:
             x_shrink = torch.matmul(A, torch.matmul(torch.cat(d_list[self.Lev - 1:], dim=1), x_shrink))
...
class Net(nn.Module):
    def __init__(self, num_features, nhid, num_classes, r, Lev, num_nodes, shrinkage='soft', sigma=1.0, dropout_prob=0.5, activation = None):
        super(Net, self).__init__()
        self.GConv1 = SVDUFGConv(num_features, nhid, r, Lev, num_nodes, shrinkage, sigma, activation = activation)
        #self.GConv2 = SVDUFGConv(nhid, num_classes, r, Lev, num_nodes, shrinkage, sigma, activation = activation)    # This may be used when do denoising
    ...
    def forward(self, data, d_list, A):
        x = data  # x has shape [num_nodes, num_input_features]

        x = self.GConv1(x, d_list, A)
        x = self.drop1(x)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sigma', type=float, default=0.001,  #0.00001,  0.005,  #0.0001,
    ...
    indices = A.nonzero()
    A0 = sparse.csr_matrix((A.data, (indices[0], indices[1])), shape=(A.shape[0], A.shape[0]))
    A = (A0 + sparse.eye(A0.shape[0]))
    row_deg_inv_sqrt = np.power(A.sum(axis=1).clip(min=1), -0.5)
    col_deg_inv_sqrt = np.power(A.sum(axis=0).clip(min=1), -0.5)
    A = diags(row_deg_inv_sqrt.flat) @ A @ diags(col_deg_inv_sqrt.flat)
    AtA = A.T @ A
    ...
    n = args.n
    r = len(DFilters)
    lobpcg_init = np.random.rand(A.shape[0], 1)
    lambda_max, _ = lobpcg(AtA, lobpcg_init)
    lambda_max = lambda_max[0]
    J = np.log(lambda_max / np.pi) / np.log(scale)
    d = get_operator2(AtA, DFilters, n, scale, J, Lev)
    ...
    ResultCSV = args.dataset + '_FastSVDExp.csv'

    mask = train_test_split(z, 1333, train_examples_per_class=20, val_examples_per_class=None,
    ...
    # Data to torch
    A = scipy_to_torch_sparse(A).to(device)
    ...
    for rep in range(num_reps):
        ...
        model = Net(X.shape[1], nhid, num_classes, r, Lev, X.shape[0], shrinkage=None, sigma=args.sigma, dropout_prob=dropout_prb, activation = activation).to(device)
