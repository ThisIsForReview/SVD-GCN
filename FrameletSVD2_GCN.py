...
class SVDUFGConv(nn.Module):
    ...
    def forward(self, x, d_list, U, Vt):
        ...
        for j in range(d_list.shape[0]):
            recons += torch.matmul(torch.transpose(Vt,0,1), ((d_list[j, :] * self.filter[j] * d_list[j, :]).unsqueeze(-1)) * torch.matmul(Vt, x))
        x_shrink = recons
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ExpNum', type=int, default='101',
    ...
    SaveResultFilename = args.dataset + 'VSVExp{0:03d}'.format(args.ExpNum)
    ResultCSV = args.dataset + '_VSV_Exp.csv'
...
#This program works well for Cora-ML dataset, achieving about 78.5 to 79.1 accuracy with single SVD layer.
#To do:
# 1. The capability in denoising such as applying shrinkage methods
# 2. How bad or good with multiple SVD layers
# 3. More tuning on hyperparameters such as scale, level, learning rate, framelets etc.