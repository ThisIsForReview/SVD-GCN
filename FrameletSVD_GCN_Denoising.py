...
class Net(nn.Module):
    def __init__(self, num_features, nhid, num_classes, r, Lev, num_nodes, dropout_prob=0.5, activation = None):
        ...
        self.GConv2 = SVDUFGConv(nhid, num_classes, r, Lev, num_nodes, activation = activation)    # This may be used when do denoising
        self.drop1 = nn.Dropout(dropout_prob)
        ## If you are using GConv2, please comment out the following four lines.
    ...
    def forward(self, data, d_list, U, Vt):
        ...
        x = self.drop1(x)
        x = self.GConv2(x, d_list, U, Vt)        # If you open this line, please comment out the following line.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--noiseLev', type=float, default=0.1,
    ...
    ResultCSV = args.dataset + '_DenoisingExp.csv'
    ...
    # Here is the place we can add noises to features X.  There are two ways to add noises:
    # Way 1:   if the features are 0 or 1, we flip the feature values between 0 and 1 according to certain percentage, this can be done by
    # ratio = args.noiseLev    # make sure this is a number between 0% - 100% in setting
    #X = nodeAttack(X, device, ratio, normal=False)

    # Way 2: If the features are normalized value, we can provide std value for Gaussian noiose by
    std = args.noiseLev      # we still use this parameter for Gaussian noise level
    X = nodeAttack(X, device, std, normal=True)
