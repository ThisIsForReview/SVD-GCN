...
def sample_per_class(random_state, labels, num_examples_per_class, forbidden_indices=None):
    ...
    return np.concatenate(
        [random_state.choice(sample_indices_per_class[class_index], num_examples_per_class, replace=False) if len(sample_indices_per_class[class_index]) >= num_examples_per_class else np.array(sample_indices_per_class[class_index])
...
# This file is from https://github.com/EdisonLeeeee/GraphData
# It works for those npz files on this site:
...
class EarlyStopping(object):
    ...
    def step(self, loss, acc, model):
        if self.best_loss is None:
            ...
            #if (loss <= self.best_loss) or (acc >= self.best_acc):
